...
/**
 * Maximum image dimensions (prevents decompression bombs)
 */
const MAX_DIMENSION = 4096;

/**
 * Maximum pixel count (4096^2 = 16,777,216 pixels)
 */
 ...
 */
export async function validateImage(buffer: Buffer, maxSizeBytes: number): Promise<ImageValidationResult> {
...
export async function sanitizeImage(buffer: Buffer): Promise<Buffer> {
    try {
        ...
        throw new Error(`Failed to sanitize image: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
}
 ...
 */
export function processWithTimeout<T>(
    promise: Promise<T>,
    timeoutMs: number
): Promise<T> {
    return Promise.race([
...
export async function validateAndSanitizeImage(
    ...
    try {
        ...
        const sanitizedBuffer = await processWithTimeout(
            sanitizeImage(buffer),
            timeoutMs
        );
